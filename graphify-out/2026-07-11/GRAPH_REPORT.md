# Graph Report - Ts-Rx  (2026-07-09)

## Corpus Check
- 50 files · ~42,412 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 572 nodes · 1257 edges · 34 communities (30 shown, 4 thin omitted)
- Extraction: 92% EXTRACTED · 8% INFERRED · 0% AMBIGUOUS · INFERRED: 102 edges (avg confidence: 0.59)
...
- `test_extract_solution_steps_removes_manual_prefix_and_safety_note()` --calls--> `_extract_solution_steps()`  [EXTRACTED]
  tests/test_execution_plan_cleanup.py → analysis_agent/main.py
- `AnalysisWorker` --uses--> `Analyzer`  [INFERRED]
  analysis_agent/worker.py → analysis_agent/analyzer.py
- `RequestIdFilter` --uses--> `JobStatus`  [INFERRED]
...
Cohesion: 0.09
Nodes (37): Analyzer, _build_structured_summary(), _extract_section_bullets(), _has_required_sections(), _normalize_optional_string(), _normalize_summary_markdown(), _parse_confidence(), AnalysisReport (+29 more)
...
Cohesion: 0.15
Nodes (13): confidence_from_report(), get_db(), AsyncSession, on_startup(), JobStatus, str, ReportStatus, AnalysisWorker (+5 more)
...
- **Why does `UptimeStatus` connect `main.py` to `schemas.py`?**
  _High betweenness centrality (0.023) - this node is a cross-community bridge._
- **Why does `Analyzer` connect `schemas.py` to `worker.py`?**
